...
sealed class MessageEvent(val message: Message) : Event {
    ...
    open class Published(message: Message) : MessageEvent(message) {
        class Failed(override val source: MqttCommand.Publish, override val throwable: Throwable?) :
            MessageEvent.Published(source.message), Failure<MqttCommand.Publish>
    }
}